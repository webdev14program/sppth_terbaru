...
class Model_adm_lain extends CI_Model
{
	...
	{
		$sql = "SELECT tahun_ajaran.id_tahun_ajaran,setting_pembayaran_lain.jenis_pembayaran,tahun_ajaran.tahun_ajaran,COUNT(*) as adm_lain FROM `setting_pembayaran_lain`
INNER JOIN group_kelas
ON group_kelas.id_groupKelas=setting_pembayaran_lain.id_groupKelas
INNER JOIN tahun_ajaran
ON tahun_ajaran.id_tahun_ajaran=setting_pembayaran_lain.id_tahun_ajaran;";
		$query = $this->db->query($sql);
		return $query->result_array();
	}

	public function detail_THAdmLain($id_tahun_ajaran)
	{
		$sql = "SELECT setting_pembayaran_lain.id_setting_pembayaran_lain,setting_pembayaran_lain.nama_pembayaran_lain,group_kelas.nama_group,setting_pembayaran_lain.nominal,tahun_ajaran.tahun_ajaran FROM `setting_pembayaran_lain`
INNER JOIN group_kelas
...
WHERE setting_pembayaran_lain.id_tahun_ajaran='$id_tahun_ajaran'
ORDER BY group_kelas.id_groupKelas ASC;";
		$query = $this->db->query($sql);
		return $query->result_array();
	}

	public function detail_Header_THAdmLain($id_tahun_ajaran)
	{
		$sql = "SELECT setting_pembayaran_lain.id_setting_pembayaran_lain,setting_pembayaran_lain.nama_pembayaran_lain,group_kelas.nama_group,setting_pembayaran_lain.nominal,tahun_ajaran.tahun_ajaran FROM `setting_pembayaran_lain`
INNER JOIN group_kelas
ON setting_pembayaran_lain.id_groupKelas=group_kelas.id_groupKelas
...
ON setting_pembayaran_lain.id_tahun_ajaran=tahun_ajaran.id_tahun_ajaran
WHERE setting_pembayaran_lain.id_tahun_ajaran='$id_tahun_ajaran'
GROUP BY tahun_ajaran.tahun_ajaran
ORDER BY group_kelas.id_groupKelas ASC;";
		$query = $this->db->query($sql);
